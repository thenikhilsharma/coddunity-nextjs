    ...
    <nav className="fixed top-0 z-50 w-full bg-black/50 backdrop-blur-sm">
      <div className="container mx-auto flex h-20 items-center justify-between px-4">
        <button onClick={() => handleNavigation("/")} className="flex items-center gap-2">
          ...
          <span className="text-xl font-bold text-white">Coddunity</span>
        </button>
        ...
        <div className="flex items-center gap-4">
          ...
          <Button onClick={() => handleNavigation("sign-up")} className="bg-white text-black hover:bg-gray-200">
            Sign Up
          </Button>
        </div>
      </div>
      <div className="h-px bg-gray-600 mx-auto" style={{ width: "calc(100% - 2rem)" }} />
    </nav>
  )
}
